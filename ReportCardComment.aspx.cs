...
public partial class ReportCardComment : PASSIS.LIB.Utility.BasePage
{
    ...
    protected void btnSearch_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            if (userRole != null)
            {
                if (userRole.RoleId == 5)
                {
                    gvComment.DataSource = new UsersLIB().RetrieveParentsChildrenFromGradeStudent(logonUser.Id);
                    gvComment.DataBind();
                }
                else
                {
                    gvComment.DataSource = new PASSIS.LIB.ClassGradeLIB().RetrieveGradeStudents(schoolId, campusId, yearId, gradeId, curSessionId);
                    gvComment.DataBind();
                }
            }
            lblErrorMsg.Text = "Kindly select or enter the comments in the table below";
    ...
    {   
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            var comm = from c in context.RpComments where c.SchoolId == logonUser.SchoolId && c.CampusId == logonUser.SchoolCampusId
                       && c.UploadedById == logonUser.Id  select c;
