...
public partial class ViewScoreBulk : PASSIS.LIB.Utility.BasePage
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            ddlSession.DataSource = new ViewScoreBulk().schSession().Distinct();
    ...
    {
        var scores = from s in context.StudentScoreRepositoryTransactions
                     where s.ClassId == yearId && s.GradeId == gradeId && s.SessionId == sessionId && s.TermId == termId && s.SubjectId == subjectId && s.CategoryId == categoryId && s.SubCategoryId == subCategoryId && s.SchoolId == schoolId && s.CampusId == campusId && s.IsCancelled == false
                     select new
                     {
        ...
        {
            scores = scores.Where(x => x.Code == Convert.ToInt64(ddlCode.SelectedValue));
        }
    ...
    {
        var scores = from s in context.StudentScoreTransactions
                     where s.ClassId == yearId && s.GradeId == gradeId && s.AcademicSessionID == sessionId && s.TermId == termId && s.SubjectId == subjectId && s.CategoryId == categoryId && s.SubCategoryId == subCategoryId && s.SchoolId == schoolId && s.CampusId == campusId && s.IsCancelled == false
                     select new
                     {
                         s.User.StudentFullName,
                         s.User.AdmissionNumber,
    ...
    private void GetCodeCA(long yearId, long gradeId, long sessionId, long termId, long subjectId, long categoryId, long subCategoryId, long schoolId, long campusId)
    {
        var scores = (from s in context.StudentScoreRepositoryTransactions
                      where s.ClassId == yearId && s.GradeId == gradeId &&
                      s.SessionId == sessionId && s.TermId == termId &&
                      s.SubjectId == subjectId && s.CategoryId == categoryId &&
                      s.SubCategoryId == subCategoryId && s.SchoolId == schoolId && s.CampusId == campusId && s.Code != null
                      select new { s.Code }).Distinct();
        ddlCode.DataSource = scores;
        ddlCode.DataBind();
        ddlCode.Items.Insert(0, new ListItem("--Select--", "0", true));
    }

    private void GetCodeExam(long yearId, long gradeId, long sessionId, long termId, long subjectId, long categoryId, long subCategoryId, long schoolId, long campusId)
    {
        var scores = (from s in context.StudentScoreTransactions
                      where s.ClassId == yearId && s.GradeId == gradeId &&
                      s.AcademicSessionID == sessionId && s.TermId == termId &&
                      s.SubjectId == subjectId && s.CategoryId == categoryId &&
                      s.SubCategoryId == subCategoryId && s.SchoolId == schoolId && s.CampusId == campusId && s.Code != null
                      select new { s.Code }).Distinct();
        ddlCode.DataSource = scores;
    ...
    protected void ddlSubCategory_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        {
            GetCodeExam(yearId, gradeId, sessionId, termId, subjectId, categoryId, subCategoryId, schoolId, campusId);
        }
        if (ddlCategory.SelectedItem.Text == "CA")
        {
            GetCodeCA(yearId, gradeId, sessionId, termId, subjectId, categoryId, subCategoryId, schoolId, campusId);
        }
    }
}