...
public partial class DeleteScores : PASSIS.LIB.Utility.BasePage
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            ddlSession.DataSource = new DeleteScores().schSession().Distinct();
            ...
            if (!isUserClassTeacher)
            {
                ...
                //PASSIS.LIB.Grade grd = getLogonTeacherGrade;
                //var getTestTemplate = from t in context.TestAssignmentScoreTemplates
                //                      where t.SchoolId == logonUser.SchoolId && t.CampusId == logonUser.SchoolCampusId && t.TeacherId == logonUser.Id
                //                      select t;
                //ddlCode.DataSource = getTestTemplate.ToList();
                //ddlCode.DataBind();
                ...
                //                      where t.SchoolId == logonUser.SchoolId && t.CampusId == logonUser.SchoolCampusId && t.SubjectTeacherId == logonUser.Id
                //                      select t;
                //ddlDescription.DataSource = getDescription.ToList();
    ...
    public void BindScore()
    {
        ...
        string templateCode = yearId.ToString() + gradeId.ToString() + subjectId.ToString() + TeacherId.ToString() + sessionId.ToString() + termId.ToString() + ddlCategory.SelectedValue.ToString() + ddlSubCategory.SelectedValue.ToString();
        PASSIS.LIB.TestAssignmentScoreTemplate getTemplateCode = context.TestAssignmentScoreTemplates.FirstOrDefault(x => x.TemplateCode == templateCode);
        ...
        if (ddlCategory.SelectedItem.Text == "CA") //if CA is selected
        {
            try
            {
                var getScores = from score in context.StudentScoreRepositoryTransactions
                                where score.TemplateId == getTemplateCode.ID && score.Code == Convert.ToInt64((ddlCode2.SelectedItem.Text))
                                ...
                                && score.ClassId == Convert.ToInt64(ddlYear.SelectedValue) && score.GradeId == Convert.ToInt64(ddlGrade.SelectedValue) && score.SubjectId == Convert.ToInt64(ddlClassSubject.SelectedValue)
                                && score.CategoryId == Convert.ToInt64(ddlCategory.SelectedValue) && score.SubCategoryId == Convert.ToInt64(ddlSubCategory.SelectedValue)
                                select score;
    ...
    protected void btnViewScores_Click(object sender, EventArgs e)
    {
        ...
        {
            lblErrorMsg.Text = "Kindly select SubCategory";
            lblErrorMsg.ForeColor = System.Drawing.Color.Red;
            lblErrorMsg.Visible = true;
            return;
        }
    ...
    protected void btnDeleteScores_Click(object sender, EventArgs e)
    {
      ...
      && x.SchoolId == logonUser.SchoolId);
        ...
        if (ddlCategory.SelectedItem.Text == "CA") //Todo Code if CA is selected
        {
            Int64 yearId = Convert.ToInt64(ddlYear.SelectedValue);
            Int64 gradeId = Convert.ToInt64(ddlGrade.SelectedValue);
            Int64 subjectId = Convert.ToInt64(ddlClassSubject.SelectedValue);
            ...
            try
            {
                ...
                var getScores = from score in context.StudentScoreRepositoryTransactions
                                where score.TemplateId == getTemplateCode.ID && score.Code == Convert.ToInt64((ddlCode2.SelectedItem.Text))
                                && score.SessionId == Convert.ToInt64(ddlSession.SelectedValue) && score.TermId == Convert.ToInt64(ddlTerm.SelectedValue)
                                 && score.ClassId == Convert.ToInt64(ddlYear.SelectedValue) && score.GradeId == Convert.ToInt64(ddlGrade.SelectedValue) && score.SubjectId == Convert.ToInt64(ddlClassSubject.SelectedValue)
                                 && score.CategoryId == Convert.ToInt64(ddlCategory.SelectedValue) && score.SubCategoryId == Convert.ToInt64(ddlSubCategory.SelectedValue)
                                select score;
                ...
                foreach (GridViewRow row in gdvTestScores.Rows)
                {
                    if (row.RowType == DataControlRowType.DataRow)
                    {
                        ...
                        PASSIS.LIB.StudentScoreRepositoryTransaction stdSc = context.StudentScoreRepositoryTransactions.FirstOrDefault(x => x.TemplateId == tempObject.ID && x.Code == Convert.ToInt64(ddlCode2.SelectedItem.Text)
                             && x.SessionId == Convert.ToInt64(ddlSession.SelectedValue) && x.TermId == Convert.ToInt64(ddlTerm.SelectedValue) && x.ClassId == Convert.ToInt64(ddlYear.SelectedValue) && x.GradeId == Convert.ToInt64(ddlGrade.SelectedValue)
                            && x.SubjectId == Convert.ToInt64(ddlClassSubject.SelectedValue));
                        if (stdSc != null)
                        {
                            PASSIS.LIB.StudentScoreRepository getScoreObj = context.StudentScoreRepositories.FirstOrDefault(x => x.AdmissionNo == lblAdmNo.Text.Trim() && x.StudentId == Convert.ToInt64(lblId.Text.ToString().Trim()) && x.TemplateId == tempObject.ID
                                 && x.SessionId == Convert.ToInt64(ddlSession.SelectedValue) && x.TermId == Convert.ToInt64(ddlTerm.SelectedValue) && x.ClassId == Convert.ToInt64(ddlYear.SelectedValue) && x.GradeId == Convert.ToInt64(ddlGrade.SelectedValue)
                            && x.SubjectId == Convert.ToInt64(ddlClassSubject.SelectedValue));

                            PASSIS.LIB.StudentScoreRepositoryTransaction getScoreTransObj = context.StudentScoreRepositoryTransactions.FirstOrDefault(x => x.AdmissionNo == lblAdmNo.Text.Trim() && x.StudentId == Convert.ToInt64(lblId.Text.ToString().Trim()) && x.TemplateId == tempObject.ID && x.Code == Convert.ToInt64(ddlCode2.SelectedItem.Text)
                                 && x.SessionId == Convert.ToInt64(ddlSession.SelectedValue) && x.TermId == Convert.ToInt64(ddlTerm.SelectedValue) && x.ClassId == Convert.ToInt64(ddlYear.SelectedValue) && x.GradeId == Convert.ToInt64(ddlGrade.SelectedValue)
                               && x.SubjectId == Convert.ToInt64(ddlClassSubject.SelectedValue));
        ...
        else if (ddlCategory.SelectedItem.Text == "Exam") //  //Todo Code if Exam is Selected
        {
            Int64 yearId = Convert.ToInt64(ddlYear.SelectedValue);
            Int64 gradeId = Convert.ToInt64(ddlGrade.SelectedValue);
            Int64 subjectId = Convert.ToInt64(ddlClassSubject.SelectedValue);
            ...
            try
            {
                ...
                foreach (GridViewRow row in gdvExamScores.Rows)
                {
                    if (row.RowType == DataControlRowType.DataRow)
                    {
                        ...
                        PASSIS.LIB.StudentScoreTransaction stdSc = context.StudentScoreTransactions.FirstOrDefault(x => x.TemplateId == tempObject.ID && x.Code == Convert.ToInt64(ddlCode2.SelectedItem.Text)
                             && x.AcademicSessionID == Convert.ToInt64(ddlSession.SelectedValue) && x.TermId == Convert.ToInt64(ddlTerm.SelectedValue) && x.ClassId == Convert.ToInt64(ddlYear.SelectedValue) && x.GradeId == Convert.ToInt64(ddlGrade.SelectedValue)
                            && x.SubjectId == Convert.ToInt64(ddlClassSubject.SelectedValue));
                        ...
                        if (stdSc != null)
                        {
                            ...
                            PASSIS.LIB.StudentScoreTransaction getScoreTransObj = context.StudentScoreTransactions.FirstOrDefault(x => x.AdmissionNumber == lblAdmNo.Text.Trim() && x.StudentId == Convert.ToInt64(lblId.Text.ToString().Trim()) && x.TemplateId == tempObject.ID && x.Code == Convert.ToInt64(ddlCode2.SelectedItem.Text)
                                && x.AcademicSessionID == Convert.ToInt64(ddlSession.SelectedValue) && x.TermId == Convert.ToInt64(ddlTerm.SelectedValue) && x.ClassId == Convert.ToInt64(ddlYear.SelectedValue) && x.GradeId == Convert.ToInt64(ddlGrade.SelectedValue)
                               && x.SubjectId == Convert.ToInt64(ddlClassSubject.SelectedValue));
    ...
    protected void ddlSubCategory_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        string templateCode = yearId.ToString() + gradeId.ToString() + subjectId.ToString() + TeacherId.ToString() + sessionId.ToString() + termId.ToString() + ddlCategory.SelectedValue.ToString() + ddlSubCategory.SelectedValue.ToString();
        PASSIS.LIB.TestAssignmentScoreTemplate getTemplateCode = context.TestAssignmentScoreTemplates.FirstOrDefault(x => x.TemplateCode == templateCode);
        //Response.Write(getTemplateCode.ID);
        if (getTemplateCode != null) {

            if (ddlCategory.SelectedItem.Text == "CA") //if CA is selected
            {
                var getDescription = from t in context.StudentScoreRepositoryTransactions
                                     where t.SchoolId == logonUser.SchoolId && t.CampusId == logonUser.SchoolCampusId && t.UploadedById == logonUser.Id && t.TemplateId == getTemplateCode.ID
            ...
            else if (ddlCategory.SelectedItem.Text == "Exam") //add code to check for extracurricular, CA, EXAM and Behavioural
            {
                var getDescription = from t in context.StudentScoreTransactions
                                     where t.SchoolId == logonUser.SchoolId && t.CampusId == logonUser.SchoolCampusId && t.UploadedById == logonUser.Id && t.TemplateId == getTemplateCode.ID
                                     && t.IsCancelled == false && t.Code != null
