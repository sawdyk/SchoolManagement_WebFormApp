...
public partial class PrintResultt_753 : PASSIS.LIB.Utility.BasePage
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            ddlAcademicSession.DataSource = new PrintResultt_753().schSession().Distinct();
            ...
            foreach (SubjectDepartment dept in deptList)
            {
                ...
                foreach (SubjectsInSchool subjects in getSubjectInClass)
                {
                    ...
                    if (subject != null)
                    {
                        ...
                        if (ddlAcademicTerm.SelectedValue == "1" || ddlAcademicTerm.SelectedValue == "2" || ddlAcademicTerm.SelectedValue == "3")
                        {
                        ...
                        if (ddlAcademicTerm.SelectedValue == "1" || ddlAcademicTerm.SelectedValue == "2" || ddlAcademicTerm.SelectedValue == "3")
                        {
                            ...
                            if (checkExist != null && checkExist.Exam == false && checkExist.CA == true) //CA only
                            {
                                ...
                                if (scoreRepoFirstTerm.Count > 0)
                                {
                                    ...
                                    {
                                        testPercentageScore += (decimal)s.PercentageScore;
                                        totalScore = testPercentageScore;
                                        break;
                                    }
                            ...
                            else if (checkExist != null && checkExist.CA == false && checkExist.Exam == true)
                            {
                                ...
                                countStdScore += stdScore.Count();
                                if (stdScore.Count > 0)
                                {
                                    foreach (PASSIS.LIB.StudentScore s in stdScore)
                                    {
                                        examPercentageScore = (decimal)s.PercentageScore;
                                        exTotal += examPercentageScore;
                                        break;
                                    }
                                }
                                else
                                ...
                                if (countStdScore > 0)
                                {
                                    totalExamObtained = exTotal / countStdScore;
                                }
                                exTotal = 0;
                                countExam = 0;
    ...
    protected void addResultSummaryPageCrownEndTerm(Document document, PASSIS.LIB.User student, UsersLIB usrDal)
    {
        ...
        long? schId = logonUser.SchoolId;
        long gradeId = new PrintResultt_753().theGradeId(student.Id).GradeId;
        string noInClass = noOfStudentInClass(schId, gradeId).Count.ToString();
        string classPosition = "";
        var posInClass = getClassPosition(student.AdmissionNumber, term, session, schoolId, yearId, gradeIdd);
        if (posInClass != null)
