...
public partial class DeleteScoresStudent : PASSIS.LIB.Utility.BasePage
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            ddlSession.DataSource = new DeleteScoresStudent().schSession().Distinct();
    ...
    protected void BindGrid(long yearId, long gradeId)
    {
        ...
        {
            PASSIS.LIB.User stdList = context.Users.FirstOrDefault(x => x.Id == studentList.StudentId);
        ...
        if (ddlCategory.SelectedItem.Text == "CA") //if CA is selected
        {
            try
            {
                ...
                var getScores = from score in context.StudentScoreRepositoryTransactions
                                ...
                                && score.CategoryId == Convert.ToInt64(ddlCategory.SelectedValue) && score.SubCategoryId == Convert.ToInt64(ddlSubCategory.SelectedValue) && score.StudentId == Convert.ToInt64(ddlStudent.SelectedValue)
    ...
    protected void btnViewScores_Click(object sender, EventArgs e)
    {
        ...
        {
            lblErrorMsg.Text = "Kindly select Student";
            lblErrorMsg.ForeColor = System.Drawing.Color.Red;
            lblErrorMsg.Visible = true;
            return;
        }
            ...
            try
            {
                ...
                foreach (StudentScoreRepositoryTransaction score in stdScore)
                {
                    PASSIS.LIB.StudentScoreRepositoryTransaction stdSc = context.StudentScoreRepositoryTransactions.FirstOrDefault(x => x.TestAssigenmentBroadSheetTemplateID == Convert.ToInt64(ddlCode.SelectedValue)
                        ...
                        && x.SubjectId == score.SubjectId && x.AdmissionNo == score.AdmissionNo && x.StudentId == score.StudentId
                        );
            ...
            try
            {
                ...
                foreach (StudentScoreTransaction score in stdScore)
                {
                    PASSIS.LIB.StudentScoreTransaction stdSc = context.StudentScoreTransactions.FirstOrDefault(x => x.BroadSheetTemplateID == Convert.ToInt64(ddlCode.SelectedValue) && x.Code == Convert.ToInt64(ddlCode2.SelectedItem.Text)
                        ...
                        && x.SubjectId == score.SubjectId && x.AdmissionNumber == score.AdmissionNumber && x.StudentId == score.StudentId
                        );
                ...
                lblErrorMsg.Text = "Student Score Deleted Successfully";
                lblErrorMsg.ForeColor = System.Drawing.Color.Red;
                lblErrorMsg.Visible = true;
                gdvExamScores.DataSource = getScores;
                gdvExamScores.DataBind();
